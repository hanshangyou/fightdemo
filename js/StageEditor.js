import { RARITY, getCharacterPool } from './GachaSystem.js';
...
export const DEFAULT_STAGES = [
    ...
    { id: 4, name: '第4关：幽灵古堡', description: '古堡中游荡着强大的亡灵', enemies: ['ghost', 'ghost', 'vampire', 'skeleton'], rewards: { gold: 300, gachaTickets: 6 }, maxTeamSize: 4 },
    { id: 5, name: '第5关：熔岩地狱', description: '炽热的地狱中潜伏着恶魔', enemies: ['demon', 'demon', 'demon', 'demon'], rewards: { gold: 400, gachaTickets: 8 }, maxTeamSize: 5 },
    { id: 6, name: '第6关：魔王城', description: '最终决战！击败魔王军团', enemies: ['demon', 'vampire', 'dragon', 'demon_king'], rewards: { gold: 600, gachaTickets: 12 }, maxTeamSize: 5 }
];
    ...
    renderStageList() {
        ...
        listContainer.innerHTML = this.stages.map(stage => {
            const enemyNames = stage.enemies.map(eid => {
                ...
                return char ? `${char.icon || '👤'}${char.name}` : eid;
            }).join(' ');
